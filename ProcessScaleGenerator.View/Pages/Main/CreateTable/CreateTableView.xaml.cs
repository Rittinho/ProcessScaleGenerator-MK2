...
public partial class CreateTableView : ContentPage
{
    ...
    {
        base.OnAppearing();
        if (BindingContext is CreateTableViewModel vm)
        {
            vm.OnPreviewTableCreated += TableGroupRender;
            vm.OnTableSaved += ClearList;
        }
    }
    ...
    {
        base.OnDisappearing();
        // CRUCIAL: Desinscrever para evitar vazamento de memória
        if (BindingContext is CreateTableViewModel vm)
        {
            vm.OnPreviewTableCreated -= TableGroupRender;
            vm.OnTableSaved -= ClearList;
    ...
    private void ClearList(bool a) => MainContainer.Clear();
